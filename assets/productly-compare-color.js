if (typeof window.compareColor === 'undefined') {
    window.compareColor = function() {
        class CompareColor extends HTMLElement {
            constructor() {
                ...
                this.popup = this;
                this.imageList = $(this.querySelector('.productly-compareColors-image'));
                this.textList = this.querySelector('.productly-compareColors-text');
            ...
            setOpenPopup(event){
                ...
                if (document.body.classList.contains('quick-view-show')) {
                    $('.productly-popup-content .productly-compare-color-popup').addClass('is-show');
                }
                else {
                    $('#MainContent .productly-compare-color-popup').addClass('is-show');
                }
            }
            ...
            setClosePopup(event){
                ...
                $('.productly-compare-color-popup').removeClass('is-show');
            }
            ...
            onBodyClickEvent(event){
                if(document.body.classList.contains('compare-color-show')){
                    if ((!this.contains(event.target)) && ($(event.target).closest('[data-open-compare-color-popup], .productly-popup-content').length === 0)){
                        this.setClosePopup(event);
                    }
                }
            }
    ...
    var quickViewShow = document.body.classList.contains('quick-view-show'),
        productCompareColor = $('.productly-productView .productView-compareColor').length;
